...
Your job is to count for everyone in queue their waiting time, and for each table 
the number of players it has served for the day.
...
K (≤100) - the number of tables, and M (< K) - the number of VIP tables. 
...
struct pairs
{
    int atime, stime, p, tag;
};
bool cmp1(pairs p1, pairs p2)
{
    return p1.atime-p2.atime;
}
bool cmp2(pairs p1, pairs p2)
{
...
int main()
{
    ...
    for(int i=0; i<N; i++)
    {
        scnaf("%d:%d:%d %d %d", &HH, &MM, &SS, &pair[i].p; &pair[i].tag);
        pair[i].atime = (HH*60+MM)*60+SS; 
    }
    sort(pair, pair+N, cmp1);
    int K, M;
    scanf("%d %d", &K, &M);
    int vip[M];
    for(int i=0; i<M; i++)
