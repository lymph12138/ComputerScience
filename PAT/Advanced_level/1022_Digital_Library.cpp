...
their titles, authors, key words of their abstracts, publishers, and published years.
...
void query(map<string, set<int>> &m, string &tmp)
{
    if(m.find(tmp) != m.end())
    {
        auto it = m[tmp].begin();
...
int main()
{
    ...
    int ID;
    string tmptitle, tmpauthor, tmpkey, tmppub, tmpyear;
    for(int i=0; i<N; i++)
    {
        scanf("%d\n", &ID);
        ...
        while(cin >> tmpkey)
        {
            key[tmpkey].insert(ID);
            char c = getchar();
            if(c == '\n')   
                break;
        }
        getline(cin, tmppub);
        pub[tmppub].insert(ID);
        getline(cin, tmpyear);
