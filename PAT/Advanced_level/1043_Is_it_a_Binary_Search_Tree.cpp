...
you are supposed to tell if it is the preorder traversal sequence of a BST or the mirror image of a BST.
Input Specification:
...
vector <int> pre, post;
bool ismirror;
void getpost(int root, int trail)
{
    if(root>trail)
        return ;
    int i = root+1;
